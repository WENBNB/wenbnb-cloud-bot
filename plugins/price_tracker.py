...
# === Branding ===
BRAND_FOOTER = "💫 Powered by <b>WENBNB Neural Engine</b> — Neural Market Feed v8.5 ⚡"
...
def price_cmd(update, context):
    try:
        ...
        # 🧠 Handle fallback for /price or /price wenbnb
        if token == "WENBNB":
        ...
        if not price:
            try:
                ...
                price = cg_data.get(token.lower(), {}).get("usd")
                source = "CoinGecko"
            except Exception:
                price = None
        ...
        if not price:
            update.message.reply_text(
                "⚠️ Could not find valid market data — please try a known token symbol or contract address.\n\n"
                f"{BRAND_FOOTER}",
                parse_mode="HTML"
            )
...
def register(dispatcher, core=None):
    ...
    print("✅ Loaded plugin: plugins.price_tracker (v8.5 WENBNB Evolving Edition)")
