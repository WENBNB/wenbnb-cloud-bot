...
# === Branding ===
BRAND_FOOTER = "💫 Powered by <b>WENBNB Neural Engine</b> — Neural Market Intelligence v8.1.8 ⚡"
...
DEFAULT_WEN = os.getenv("WEN_TOKEN_ADDRESS", "0x78525f54e46d2821ec59bfae27201058881b4444")
...
def price_cmd(update, context):
    try:
        ...
        # 🧠 Default token fallback (with contract support)
        if context.args:
            token = context.args[0].strip().upper()
        else:
            token = DEFAULT_WEN
            update.message.reply_text(
                f"💡 No token specified — showing default <b>WENBNB</b> market data.",
                parse_mode="HTML"
            )
...
def register(dispatcher, core=None):
    ...
    print("✅ Loaded plugin: plugins.price_tracker (v8.1.8 Default Fallback Fix Edition)")
